import type { EditRedeemedById } from 'types/graphql'
...
export const Success = ({ redeemed }: CellSuccessProps<EditRedeemedById>) => {
  const [updateRedeemed, { loading, error }] = useMutation(UPDATE_REDEEMED_MUTATION, {
    onCompleted: () => {
  ...
  return (
    <div className="rw-segment">
      <header className="rw-segment-header">
        <h2 className="rw-heading rw-heading-secondary">Edit Redeemed {redeemed.id}</h2>
      </header>
      <div className="rw-segment-main">
        <RedeemedForm redeemed={redeemed} onSave={onSave} error={error} loading={loading} />
      </div>
    </div>
  )
}
